...
class ProblemSuites(dbmodel, Helper):
  ...
  # force suite to be unique on a per-problem basis
  __table_args__ = (dbunicon("pid", "suite"), )
  
  __tablename__ = "problem_suites"
  
class ProblemCases(dbmodel, Helper):
  id = dbcol(dblong, primary_key = True)
  psid = dbcol(dblong, dbforkey(ProblemSuites.id, onupdate = "CASCADE", ondelete = "CASCADE"), nullable = False)
  case = dbcol(dbint, nullable = False)
  
  # force case to be unique on a per-suite basis
  __table_args__ = (dbunicon("psid", "case"), )
