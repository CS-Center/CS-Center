...
inline void do_error(const char* file, const char* expr, int line, const char* func) {
  ...
  fprintf(stderr, msg, file, line, func, expr, r);
  fprintf(stderr, "\nErrno is %d", errno);
...
#define PUSH_STACK(fn) try{st;}catch(exception& e){print_stack_elem(__FILE__, #st, __LINE__, __PRETTY_FUNCTION__);throw e;}